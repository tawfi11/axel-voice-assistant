...
def alterSpotifyState(speech, client): ###! CREATE ALTER SPOTIFY STATE !###
    try:
        ...
        if any(phrase in speech for phrase in ['go back', 'previous track']):
            ...
            return ("Playing previous track", speech)
        
        if any(phrase in speech for phrase in ['skip track', 'next track', 'skip song', 'skip track']):
            client.next_track()
            return ("Playing next track", speech)

        if speech.endswith('on shuffle'):
            client.shuffle(True)
            speech = speech.split()
            speech = speech[0:len(speech)-2]
            speech = ' '.join(speech)
        elif speech.endswith('off shuffle'):
            client.shuffle(False)
            speech = speech.split()
            speech = speech[0:len(speech)-2]
            speech = ' '.join(speech)
        
        if speech.endswith('on repeat'):
            client.repeat('context')
            speech = speech.split()
            speech = speech[0:len(speech)-2]
            speech = ' '.join(speech)
        elif speech.endswith('off repeat'):
            client.repeat('off')
            speech = speech.split()
            speech = speech[0:len(speech)-2]
            speech = ' '.join(speech)
...
def parseSpotify(speech = "", engine=None, source=None, r=None,user=None): ###! MAIN SPOTIFY !###
    try:
        ...
        if state == 0 and probability >= 0.7: return "Turned on spotify"
        elif state == 6 and probability >= 0.7: return shuffle(speech, client)
        elif state == 7 and probability >= 0.7: return repeat(speech, client)
        ###? STEP 2: See if spotify should just be turned on ?###
        ...
        #5: Add to queue
        client.repeat('context')
        alterTask, speech = alterSpotifyState(speech, client) ###* Alter spotify state is possible *###
        ...
        ###* Returns (songURI, playlist/album/artist URI, queue query, return msg)
        if state == 1: returnMsg = playSong(speech,nlp,client)
        elif state == 2: returnMsg = playPlaylist(speech,nlp,client)
        elif state == 3: returnMsg = playArtist(speech,client)
        elif state == 4: returnMsg = playAlbum(speech,client)
        elif state == 5: returnMsg = addToQueue(speech, client, False)
...
if testing == True:
    ...
    while typing == True:
        ...
        if speech == "": typing = False
        else: print(parseSpotify(speech=speech))
    r = sr.Recognizer()
    ...
    with mic as source:
        ...
        while True:
            ...
            try:
                ...
                continue
            engine.say(parseSpotify(speech=speech))
            engine.runAndWait()
'''